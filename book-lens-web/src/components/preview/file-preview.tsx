...
export function FilePreview() {
  ...
  const loadFiles = React.useCallback(async () => {
    ...
    try {
      ...
      const files = detail.files
        .filter((f) => f.is_markdown || f.is_html || f.is_pdf || f.is_epub || f.is_text)
        .map((f) => f.name)
        .sort((a, b) => {
  ...
  const activeIsEpub =
    activeTab != null && activeTab.toLowerCase().endsWith(".epub");
    ...
    >
      ...
      <div className="flex min-w-0 shrink-0 items-center gap-2 border-b border-border/60 bg-card/50 px-3 py-2 backdrop-blur">
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          ...
          />
          {(activeIsHtml || activeIsPdf || activeIsEpub) && paper && activeTab && (
            <IconBtn
              label="在新标签打开"
