...
export const BACKEND_BASE =
  typeof window !== "undefined"
    ? (process.env.NEXT_PUBLIC_BACKEND_URL ??
      (process.env.NODE_ENV === "development" ? "http://localhost:8767" : ""))
    : "";
...
export const api = {
  async listPapers(): Promise<{ papers: Paper[] }> {
    const { books } = await jsonFetch<{ books: Paper[] }>("/api/books");
    return { papers: books };
  },
  ...
  async renamePaper(oldName: string, newName: string) {
    ...
    return jsonFetch<{ ok: boolean; old_name: string; new_name: string }>("/api/rename-book", {
      method: "POST",
      body: fd,
    });
  },
